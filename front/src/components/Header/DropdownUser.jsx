...
const DropdownUser = () => {
  ...
  useEffect(() => {
    const clickHandler = (e) => {
      ...
      if (
        !dropdownOpen ||
        dropdown.current.contains(e) ||
        trigger.current.contains(e)
      )
        return;
  ...
  useEffect(() => {
    const keyHandler = (e) => {
      e.stopPropagation()
      if (!dropdownOpen || e.key !== 27) return;
      setDropdownOpen(false);
    };
  ...
  return (
    <>
      {/* {data?.user_email === " " ? navigate("/signin") : ( */}
        <div className="relative">
          <Link
            ...
            to="#"
          >
            <span className="flex flex-row hidden text-right lg:block">
              <span className="block flex gap-2 text-sm font-medium text-black dark:text-white">
                {auth?.user && auth?.user?.user_email}
              </span>
              <span className="block flex gap-2 text-sm font-medium text-black dark:text-white">
                 {auth?.user && auth?.user?.user_firstname} {auth && auth?.user?.user_lastname}
              </span>
              <span className="block text-xs">{auth?.user && auth?.user?.user_country}</span>
            </span>

            <span className="h-12 w-12 rounded-full">
              <img src={`@/images/${auth?.user && auth?.user?.user_profilepicture}`} alt="User" />
            </span>
